...
def plot_data_preview(X: np.ndarray, y: np.ndarray, snr: np.ndarray, class_names: List[str], cfg: Config) -> None:
    ...
    # Constellation grid stays separated by class.
    ...
    for ax in axes:
        ax.axis("off")
    for ax, (_, label, sig) in zip(axes, examples):
        ax.scatter(np.real(sig), np.imag(sig), s=5, alpha=0.35)
        ax.set_title(label)
        ...
        ax.axis("equal")
        ax.axis("on")
    fig.suptitle("Constellation / IQ scatter examples")
    save_or_show(fig, "02_constellation_grid", cfg)
...
def ask_for_data_path(default: str) -> str:
    ...
    try:
        ...
        root = tk.Tk()
        root.withdraw()
        messagebox.showinfo("ML_POF_V14", "Choose your dataset folder or one signal file.")
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    cfg = parse_args(argv)
    set_seed(cfg.seed)
    print_header("ML_POF_V14 Universal POF / SDR Signal Classifier")
